...
export default function IdeaCard({ idea }: IdeaCardProps) {
  ...
  return (
    <div
      className={`relative flex flex-col w-[320px] bg-[var(--bg-card)] border ${isTopVoted ? "border-[#7c5cfc] shadow-[0_0_15px_rgba(124,92,252,0.4)]" : isSelected ? "border-[#7c5cfc]" : "border-[var(--border)]"} rounded-2xl p-5 shadow-lg overflow-hidden transition-all duration-300 pointer-events-none`}
      style={{
    ...
    >
      ...
      <div className="flex justify-between items-start mb-2 gap-2 pointer-events-none">
        <h3 className="font-bold text-lg leading-tight break-words flex-1 pointer-events-auto">
          {idea.title}
        </h3>
        <button
          onPointerDown={handleDelete}
          className="text-red-400 hover:text-red-500 hover:bg-red-500/10 p-1.5 rounded transition-colors flex-shrink-0 pointer-events-auto"
          title="Delete Idea"
        >
          <Trash2 size={16} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 mb-4 select-text pointer-events-none">
      ...
      {/* Primary Actions */}
      <div className="flex items-center gap-2 mt-auto pt-4 border-t border-[var(--border)] pointer-events-auto">
        <button
      ...
      {/* AI Actions */}
      <div className="flex gap-2 mt-3 pt-3 border-t border-[var(--border)] pointer-events-auto">
        <button
          onPointerDown={handleExpand}
          disabled={isExpanding}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs text-[#a78bfa] hover:bg-[#a78bfa]/10 px-2.5 py-1.5 rounded-md transition-colors font-medium border border-[#a78bfa]/30 disabled:opacity-50 pointer-events-auto"
        >
        ...
        <button
          ...
          disabled={isMapping}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs text-[#38bdf8] hover:bg-[#38bdf8]/10 px-2.5 py-1.5 rounded-md transition-colors font-medium border border-[#38bdf8]/30 disabled:opacity-50 pointer-events-auto"
        >
          {isMapping ? <Loader2 size={14} className="animate-spin" /> : <Network size={14} />}
          🕸️ Map
        </button>
      </div>
      ...
      {expansion && (
        <div
          className="mt-4 pt-4 border-t border-[var(--border)] text-xs text-[var(--text-secondary)] space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar select-text pointer-events-auto"
        ...
        >
          ...
          <div><strong className="text-[#a78bfa] block mt-2 mb-0.5">Solution:</strong> <span className="text-[var(--text-primary)]">{expansion.solution}</span></div>
